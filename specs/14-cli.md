...
| `minicrond init` | scaffold `minicron.toml` + example jobs dir (detects crontabs/compose files and mentions them) | 0/1 |
...
| `minicrond stop` | graceful daemon stop (service-aware: systemd when managed, else SIGTERM to PID from lock) | 0/1 |
...
MINICRON_URL=https://box:7423 MINICRON_TOKEN=… minicrond run backup-db
```
