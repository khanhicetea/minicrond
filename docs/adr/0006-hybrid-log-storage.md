...
3. **Crash recovery**: buffer directories left without a writer at startup are
   ...
   in-progress chunk is salvaged up to the last intact frame. The sweep also
   runs at the worker flush cadence to retry failed final archival.
...
- The data directory holds two databases; copying `minicron.db` alone no longer
  ...
  still applies.
- `log_max` defaults to 100 MiB and bounds raw bytes still in the file buffer;
  completed transfers release capacity. Archive retention is governed by
  `logs.db_keep_for` and per-run retention, not by a disk-space quota.
